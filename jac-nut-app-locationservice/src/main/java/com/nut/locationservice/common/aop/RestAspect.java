package com.nut.locationservice.common.aop;
...
public class RestAspect {
     ...
     * 定义切入点 就是需要拦截的切面
     */
    @Pointcut("execution(public * com.nut.locationservice.app.controller.*.*(..))")
    ...
    public Object printResponseDatagram(ProceedingJoinPoint joinPoint) {
        try {
            ...
            if (e instanceof ParamException) {
                ...
                response = new HttpCommandResultWithData(ECode.BUSINESS_FAILURE.code(), ((BusinessException) e).getErrorMessage());
            } else {
            ...
            //将自定义异常交给spring全局异常处理
            if (e instanceof  BusinessException){
                ExceptionUtil.result(((BusinessException) e).getCode(),e.getMessage());
            }
            return response;
        }
    }
