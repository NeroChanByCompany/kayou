...
@Data
@NutFormValidator
@ApiModel("app司机与车辆绑定Entity")
public class CarDriverBindForm extends BaseForm {
    /**
     * 车队ID
     */
     ...
     * 司机用户ID（支持半角逗号分隔）
     */
    /*@NotBlank(message = "司机用户ID不能为空", groups = {GroupCommand.class})
    private String driverId;*/

    @NotBlank(message = "主驾司机用户ID不能为空")
    @ApiModelProperty(name = "masterDriverId" , notes = "主驾驶司机ID" , dataType = "String")
    private String masterDriverId;

    @ApiModelProperty(name = "copilotDriverId" , notes = "副驾驶用户ID" , dataType = "String")
    private String copilotDriverId;


    @Override
    public String toString() {
        return "CarDriverBindCommand{" +
                "teamId=" + teamId +
                ", multipleFlag=" + multipleFlag +
                ", carId='" + carId + '\'' +
                ", masterDriverId='" + masterDriverId + '\'' +
                ", copilotDriverId='" + copilotDriverId + '\'' +
                "} " + super.toString();
    }
